...
class DynamicMemoryCell(tf.contrib.rnn.RNNCell):
    ...
    @staticmethod
    def get_gate(state_j, key_j, inputs, v=None, prev_a=None):
        """
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__, initializer=self._initializer):
            ...
            for j, state_j in enumerate(state):  # Hidden State (j)
                ...
                if j != 0:
                    reuse = True
                with tf.variable_scope("entnet_gru", reuse=reuse) as _:
                    ...
                    w_c = tf.get_variable("w_c", [self._num_units_per_block * 2, self._num_units_per_block])
                    b_c = tf.get_variable(
...
class DelayedEntNetSentihood(object):
    def __init__(
            self,
            batch_size,
            vocab_size,
        ...
        if self._l2_final_layer > 0:
            final_layer_weights = [tf.nn.l2_loss(v) for v in trainable_variables if 'R:0' in v.name]
    ...
    def _build_vars(self):
        ...
        self._nil_vars = {self._embedding.name}
    ...
    @staticmethod
    def _get_mini_batch_start_end(n_train, batch_size=None):
        """
        get mini batch start and end
        :param n_train: int, number of training instances
        :param batch_size: int (or None if full batch)
        :return: list of tuples of (start, end) of each mini batch
        """
    ...
    @staticmethod
    def _sentence_length(sentences):
        """
        get sentence length
        :param sentences: (None, sentence_len, embedding_size)
        :return:
        """
